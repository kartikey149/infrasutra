    ...
    try {
      const res = await authFetch(`${API_BASE}/schedule/activities/${editingActivity.activity_id}`, {
        ...
        body: JSON.stringify({
          ...
          percent_complete: parseInt(editFormData.percent_complete, 10),
        }),
      });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">
            <Database size={16} /> Active SQLite WBS Database
          </div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">
            Schedule Explorer
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Filtered to: <strong className="text-slate-800">{activeProject?.name}</strong> ({activities.length} activities)
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-3 items-center justify-between">
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Search by Activity ID, task name, zone..."
            value={search}
        ...
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto text-xs">
          <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl">
            ...
            <span className="font-bold text-slate-600">Discipline:</span>
            <select
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase text-[10px] tracking-wider">
              <tr>
                ...
                <th className="py-3.5 px-4">Task Name & WBS</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-slate-400">
                    Loading Primavera activities from SQLite...
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-slate-400">
                    No matching activities found for {activeProject?.name}.
                  </td>
                </tr>
              ) : (
                filtered.map((act) => {
                  ...
                  return (
                    <tr key={act.activity_id} className="hover:bg-slate-50/70 transition">
                      ...
                      <td className="py-3 px-4 font-mono text-[11px] whitespace-nowrap">
                        <div>{act.planned_start}</div>
                        <div className="text-slate-400">to {act.planned_end}</div>
                      </td>
                      <td className="py-3 px-4 font-mono text-[11px] whitespace-nowrap">
                        {act.actual_start ? (
                          <div>
                            <div>{act.actual_start}</div>
                            <div className="text-slate-400">{act.actual_end ? `to ${act.actual_end}` : '(ongoing)'}</div>
                          </div>
                        ) : (
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-[11px]">{act.percent_complete}%</span>
                        </div>
                      </td>
